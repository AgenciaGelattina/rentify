...
const formValidations = yup.object().shape({
    ...
    label: yup.string().required("Escriba el estado del propiedad."),
});
...
const TypeData: React.FC<ITypeDataProps> = ({ type, open, setTypeData, getTypes }) => {
    ...
    const onFormSubmit = (data: FieldValues) => {
        fetchData.post('/properties/statuses/status.php', data, (response: TCallBack) => {
    ...
    return (<RspDialog open={open} maxWidth="sm" onClose={() => setTypeData(defaultTypeDataState)}>
        <RspDialogTitle title={type.id > 0 ?  'EDITAR ESTADO' : 'NUEVO ESTADO'} onClose={() => setTypeData(defaultTypeDataState)} />
        <DialogContent>
            <Grid container spacing={2} sx={{ marginTop: '1rem' }}>
                <Grid size={12}>
                    <Controller name="label" control={control} render={({ field }) => {
                        return <TextField id="title" label="Estado de la Propiedad" type="text" {...field} {...fieldError(errors.label)} onChange={(e) => field.onChange(e)} fullWidth />
                    }} />
                </Grid>
            </Grid>
        </DialogContent>
