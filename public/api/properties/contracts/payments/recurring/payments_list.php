...
if (METHOD === 'GET') {
    ...
    $query = "SELECT cp.id,cp.amount,cp.contract,pc.currency,IF(cp.recurring > 0, JSON_OBJECT('id',cp.recurring,'label',crch.label), NULL) AS recurring,cp.date,cp.clarifications FROM contracts_payments AS cp LEFT JOIN contracts_recurring_charges AS crch ON crch.id = cp.recurring LEFT JOIN property_contracts AS pc on pc.id = cp.contract WHERE cp.contract = $contract_id ORDER BY cp.date";
